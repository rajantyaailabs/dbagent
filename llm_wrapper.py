...
class LLMWrapper:
    ...
    def __str__(self) -> str:
        return f"LLMWrapper(provider={self.model_provider}, model={self.model_name})"
...
def get_available_models() -> Dict[str, List[str]]:
    ...
    return {
        ...
        "powerful": ["Claude Sonnet 3.7 Thinking", "Gemini 2.5 Pro", "Gemini 2.0 Flash Thinking"],
    }


def validate_model(model_name: str) -> bool:
    """Validate if model name is supported"""
    return any(model_name == config["model"] for config in LLM_API_MAPPING.values())
